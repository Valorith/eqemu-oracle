...
class InstallerTest(unittest.TestCase):
    def test_install_home_local_plugin_falls_back_to_legacy_home_marketplace(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("eqemu_oracle.installer.subprocess.run") as run_mock:
                ...
                result = installer.install_home_local_plugin(home=home, source_plugin_root=source_root)
    ...
    def test_install_home_local_plugin_uses_codex_desktop_marketplace_when_available(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            home = Path(temp_dir) / "home"
            source_root = Path(temp_dir) / "source" / "eqemu-oracle"
            _seed_plugin_root(source_root)
    ...
    def test_install_home_local_plugin_migrates_legacy_local_overrides_into_codex_target(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            home = Path(temp_dir) / "home"
            source_root = Path(temp_dir) / "source" / "eqemu-oracle"
            _seed_plugin_root(source_root)
    ...
    def test_install_home_local_plugin_preserves_local_overrides_in_place(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            home = Path(temp_dir) / "home"
            source_root = Path(temp_dir) / "source" / "eqemu-oracle"
            _seed_plugin_root(source_root)
